...
mod tests {
    ...
    async fn declare_deploy_and_invoke_sierra_class_starknet_0_13_1(#[case] version: RpcVersion) {
        ...
        // declare test class
        let declare_transaction = declare_transaction(account_contract_address, transaction_nonce!("0x0"));
        // deploy with universal deployer contract
        let deploy_transaction =
            deploy_transaction(account_contract_address, universal_deployer_address, transaction_nonce!("0x1"));
    ...
    async fn declare_deploy_and_invoke_sierra_class_starknet_0_13_2_1(#[case] version: RpcVersion) {
        ...
        let deploy_transaction =
            deploy_transaction(account_contract_address, universal_deployer_address, transaction_nonce!("0x1"));
        // invoke deployed contract
        let invoke_transaction = invoke_transaction(account_contract_address, transaction_nonce!("0x2"));
        // do the same invoke with a v0 transaction
        let invoke_v0_transaction = invoke_v0_transaction();
        // do the same invoke with a v3 transaction
        let invoke_v3_transaction = invoke_v3_transaction(account_contract_address, transaction_nonce!("0x3"));
